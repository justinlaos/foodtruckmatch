defmodule FoodtruckmatchWeb.TruckLive.FormComponent do
  ...
  def update(%{truck: truck} = assigns, socket) do
    changeset = Location.change_truck(truck)
  ...
  def handle_event("validate", %{"truck" => truck_params}, socket) do
    changeset =
      ...
      |> Location.change_truck(truck_params)
  ...
  defp save_truck(socket, :edit, truck_params) do
    case Location.update_truck(socket.assigns.truck, truck_params) do
      {:ok, truck} ->
  ...
  defp save_truck(socket, :new, truck_params) do
    case Location.create_truck(truck_params) do
      {:ok, truck} ->
        notify_parent({:saved, truck})
